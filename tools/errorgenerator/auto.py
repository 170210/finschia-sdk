...
if __name__ == "__main__":
    ...
    files_with_name = find_files_with_name(target_path, error_file)
    ...
    for file_path in files_with_name:
    ...
    with open(target_path+"/errors.md", "w") as file:
        ...
        for module_name, paths in module_with_paths.items():
        ...
        for module_name, paths in module_with_paths.items():
            ...
            for path in paths:
                const_dict, error_dict = read_file(path)
                param_path = path.replace("errors.go", "key.go")
                moduleName=get_moduleName(param_path)
                if not(moduleName):
                    param_path = path.replace("errors.go", "keys.go")
                    moduleName=get_moduleName(param_path)
